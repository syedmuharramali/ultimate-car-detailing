...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon 
                      size={16} 
                      className={`transition-colors ${isActive ? "text-gold" : "text-bone/50"}`} 
                    />
                    {link.label}
                    {isActive && (
        ...
        <div className="flex items-center gap-3 relative z-50">
          <MagneticAnchor
            ...
            className="flex items-center gap-2 rounded-full border border-gold/60 px-4 py-2 text-sm font-medium text-gold transition-colors hover:bg-gold hover:text-graphite"
          >
            <Phone size={15} />
          ...
          <button
            ...
            className="p-2 text-bone md:hidden hover:text-gold transition-colors"
      ...
      >
        <div className="flex flex-col items-center gap-6 py-8 px-6 bg-graphite/95 backdrop-blur-md">
          {LINKS.map((link) => {
            const Icon = link.icon;
            return (
              <NavLink
          ...
          <MagneticAnchor
            href={`tel:${PHONE.replace(/-/g, "")}`}
            className="mt-2 flex items-center gap-2 rounded-full border border-gold/60 px-6 py-3 text-sm font-medium text-gold transition-colors hover:bg-gold hover:text-graphite"
          >
            <Phone size={16} />
            <span>{PHONE}</span>
          </MagneticAnchor>
